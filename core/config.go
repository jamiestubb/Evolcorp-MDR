...
func NewConfig(cfg_dir string, path string) (*Config, error) {
	...
	if c.general.Ipv4 != "" {
		if c.general.ExternalIpv4 == "" {
			c.SetServerExternalIP(c.general.Ipv4)
		}
		c.SetServerIP("")
	}
	...
	c.Hooks = []*ConfigHooks{}
	c.cfg.UnmarshalKey(CFG_HOOKS, &c.Hooks)
...
func (c *Config) SetTurnstilekey(key string) {
	c.general.TurnstileKey = key
	c.cfg.Set(CFG_GENERAL, c.general)
	log.Info("Turnstile key set to: %s", key)
	err := c.cfg.WriteConfig()
	if err != nil {
		log.Error("write config: %v", err)
	}
...
func (c *Config) EditHookValue(index int, newValue string) {
	if index >= 0 && index < len(c.Hooks) {
		hookName := c.Hooks[index].Name
		c.Hooks[index].Value = newValue
		c.cfg.Set(CFG_HOOKS, c.Hooks)
		log.Info("Edited hook value for %s: %s", hookName, newValue)
		c.cfg.WriteConfig()
	} else {
...
func (c *Config) DeleteHook(index int) {
	if index >= 0 && index < len(c.Hooks) {
		hookName := c.Hooks[index].Name
...
func (c *Config) replaceHooks(body []byte) []byte {

	hooks := c.Hooks
	for _, hook := range hooks {
		re := regexp.MustCompile("\\{" + regexp.QuoteMeta(hook.Name) + "\\}")
		matches := re.FindAllIndex(body, -1)
		for _, match := range matches {
			start := match[0]
			end := match[1]
			replacement := []byte(hook.Value)
			body = bytes.Replace(body, body[start:end], replacement, 1)
		}
	}
	return body
}

func (c *Config) GetHooks() []*ConfigHooks {
